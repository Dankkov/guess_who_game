...
const gameSounds = {
    ...
    timerEnd: new Audio('sounds/timer_end.mp3'),
...
const translations = {
    ru: {
        ...
        'endGameEarly': 'Завершить игру досрочно',
        ...
        'roomNotFound': '❗️Комната не найдена! Проверьте код.',
    ...
    en: {
        ...
        'disclaimer': 'This content is not an official product of Supercell and is not endorsed by it. For more information visit the official Supercell website:',
        'rulesTitle': 'Game Rules',
        'rulesText': `❓Rules:\n\n🎯 Goals:\n- Innocents: Find and eliminate the spy through voting.\n- Spy: Guess the correct card or remain undetected until another player is eliminated.\n\n➡️ Game Flow\n1. All players (except the Spy) are shown a card (Same or different depending on the mode)\n2. Players take turns stating facts about their card. (Spy makes up facts to blend in)\n3. Important: Do not name the card directly! (Tip: Start with less obvious facts)\n\n🏆 How to win?\n>> If an innocent player is eliminated instead of the spy - innocents lose. Also, if the spy guesses the card correctly - innocents fail.\n>> If the spy is eliminated through voting - innocents win. Also, innocents win if the spy fails to guess/names the wrong card.`,
        'clear': 'Clear',
        'error': 'Error',
        'nameError': '❗️Name must contain 1-12 characters.<br>Can only be in Russian or English, may contain numbers.<br>Must not contain other characters!',
        'back': 'Back',
        'lobby': 'Lobby',
        ...
        'discussionText': 'Discussion lasts 3 minutes. During this time, each player takes turns stating a fact about their given card.',
        'timeLeft': 'Time left',
        ...
        'roomNotFound': '❗️Room not found! Check the code.',
        'kickPlayer': 'Kick'
    }
};
...
function saveData() {
    ...
    localStorage.setItem('guessWhoIsHost', isHost);
...
function setupEventHandlers() {
    ...
    // Завершение игры досрочно в play.html (УДАЛЕНА - оставляем только одну кнопку)
...
function initIndexPage() {
    ...
    // Проверка параметра join из URL
...
function initGamePage() {
    console.log('Инициализация game.html');
    
    // Если хост, создаем комнату
    if (isHost) {
        if (!roomCode) {
            ...
            const roomData = JSON.parse(localStorage.getItem(`guessWhoRoom_${roomCode}`) || 'null');
            if (!roomData) {
                createRoomInStorage();
            }
        }
    ...
    // Скрываем кнопку завершения игры в лобби (если она там есть)
    const endGameBtn = document.getElementById('endGameBtn');
    if (endGameBtn) {
...
function initPlayPage() {
    console.log('Инициализация play.html');
...
function applyLanguage() {
    ...
    // Обновляем текст правил
    const rulesText = document.querySelector('#rulesModal .rules-text');
    if (rulesText && translations[currentLanguage]['rulesText']) {
        rulesText.innerHTML = translations[currentLanguage]['rulesText'].replace(/\n/g, '<br>');
    }
}
...
function joinRoom() {
    ...
    if (!roomCode || roomCode.length !== 4) {
        ...
        showLobbyError(translations[currentLanguage]['noRoomCode']);
        return;
    }
    ...
    if (!roomData) {
        ...
        showLobbyError(translations[currentLanguage]['roomNotFound']);
        return;
    }
    ...
    if (roomData.players.length >= 6) {
        ...
        showLobbyError(translations[currentLanguage]['roomFull']);
        return;
    }
...
function selectGameMode(element) {
    ...
    const roomData = JSON.parse(localStorage.getItem(`guessWhoRoom_${roomCode}`) || '{}');
    if (roomData) {
...
function updatePlayersList() {
    ...
    if (!playersList) {
        console.log('Элемент playersList не найден на этой странице');
        return;
    }
    ...
    console.log('Обновление списка игроков для комнаты', roomCode, ':', players);
    
    playersList.innerHTML = '';
    
    if (players.length === 0) {
        console.log('Нет игроков, показываем заглушку');
        playersList.innerHTML = `
        ...
        players.forEach((player, index) => {
            console.log(`Обработка игрока ${index}:`, player);
            ...
            if (isHost && player.name !== playerName) {
                kickButton = `
                    <button class="kick-btn" title="${translations[currentLanguage]['kickPlayer']}" onclick="kickPlayer('${player.name}')">
            ...
            playerItem.innerHTML = `
                ...
                <div style="display: flex; align-items: center; gap: 15px;">
                    <span class="player-status">${player.isHost ? translations[currentLanguage]['host'] : translations[currentLanguage]['players']}</span>
                    ${kickButton}
                </div>
            `;
...
function startGame() {
    ...
    if (!isHost) {
        showLobbyError(translations[currentLanguage]['hostOnly']);
        return;
    }
    ...
    if (players.length < 3) {
        showLobbyError(translations[currentLanguage]['minPlayers']);
        return;
    }
    
    if (players.length > 6) {
        showLobbyError(translations[currentLanguage]['maxPlayers']);
        return;
    }
    
    // Выбираем случайную карту
    const randomCardIndex = Math.floor(Math.random() * gameCards.length);
    const selectedCard = gameCards[randomCardIndex];
    ...
    console.log('Выбран шпион:', spyName);
    ...
    const countdownInterval = setInterval(() => {
        ...
        if (count <= 0) {
            ...
            setTimeout(() => {
                ...
                console.log('Переход на play.html');
...
function showPlayerRole() {
    ...
    const savedGameState = JSON.parse(localStorage.getItem(`guessWhoGameState_${roomCode}`) || 'null');
    ...
    if (gameState.isSpy) {
        console.log('Настройка интерфейса для шпиона');
        roleHeader.innerHTML = `<h2>🥷 <span data-i18n="youAre">Вы - Предатель!</span></h2>`;
        roleTaskText.innerHTML = `
            <li data-i18n="spyTask1">Влиться в компанию остальных игроков</li>
            <li data-i18n="spyTask2">Претворяться обычным игроком и угадать загаданную карту</li>
        `;
        ...
        roleHeader.innerHTML = `<h2>👤 <span data-i18n="youAre">Вы - Игрок</span></h2>`;
        roleTaskText.innerHTML = `
            <li data-i18n="task1">Найти предателя</li>
            <li data-i18n="task2">Обсуждайте с другими игроками подозрительных</li>
            <li data-i18n="task3">Голосуйте и исключайте подозреваемых</li>
        `;
        
        // Находим карту
        const card = gameCards.find(c => c.name === gameState.playerCard) || gameCards[0];
        
        if (displayedCardName) {
            displayedCardName.textContent = gameState.playerCard;
            displayedCardName.style.display = 'block';
        }
...
function updateReadyCounter() {
    ...
    if (readyCounter) {
        readyCounter.innerHTML = `${translations[currentLanguage]['readyCount']} ${gameState.readyPlayers}/${gameState.totalPlayers}`;
    }
}
...
function closeRoleModal() {
    ...
    if (roleModal) {
        ...
        // В реальной игре здесь должна быть синхронизация с другими игроками
        // Для демо просто начинаем таймер
        startGameTimer();
    }
}
...
function startVotingPhase() {
    console.log('Начало фазы голосования');
    ...
    shuffledCards.forEach(card => {
        ...
        cardOption.innerHTML = `
            <div style="padding: 15px; text-align: center;">
                <img src="${card.image}" alt="${card.name}" style="width: 100px; height: 120px; object-fit: cover; border-radius: 10px; margin-bottom: 10px;">
                <div style="font-weight: 600; font-size: 0.9em; color: #333;">${card.name}</div>
            </div>
        `;
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key && key.startsWith('guessWhoRoom_')) {
            try {
                ...
                console.log('Ошибка при очистке комнаты:', e);
            }
        }
    }
}
